...
function IssuingMedicalFitnessContent() {
  ...
  const [steps, setSteps] = useState<ProcessStep[]>([
    ...
    {
      id: "prepare",
      title: "Preparing Medical Fitness Certificate payload",
      status: "pending",
    },
    {
      id: "issue",
      title: "Issuing Medical Fitness Certificate credential",
      status: "pending",
    },
    {
      id: "email",
    ...
    const issueCredential = async () => {
      try {
        ...
        updateStepStatus(
          "prepare",
          "completed",
          `Medical fitness data for ${studentName} prepared`
        );
        moveToNextStep();
        ...
        try {
          response = await fetch("/api/medical-fitness/issue", {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          ...
          const errorMessage = result.message || result.error || "Failed to issue Medical Fitness Certificate";
        ...
        router.push(
          `/medical/fitness/success?email=${encodeURIComponent(data.email)}&phone=${encodeURIComponent(data.phoneNumber)}&name=${encodeURIComponent(fullName)}`
        );
      } catch (err: any) {
        console.error("Unexpected error issuing Medical Fitness Certificate:", err);
  ...
  const getStepIcon = (status: ProcessStep["status"]) => {
    switch (status) {
      ...
      case "in-progress":
        return (
          <svg
            className="w-6 h-6 text-teal-600 animate-spin"
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-teal-600 to-cyan-600 px-6 py-6">
              <h2 className="text-2xl font-bold text-white">
                Issuing Medical Fitness Certificate
              </h2>
              ...
              {!error ? (
                <>
                  ...
                  <div className="mt-8 bg-teal-50 border border-teal-200 rounded-lg p-4">
                    <div className="flex gap-3">
                      <div className="flex-shrink-0 text-teal-600">ℹ️</div>
                      <div className="text-sm text-teal-800">
                        <p className="font-semibold mb-1">What's happening?</p>
                        <p>
                          We're securely issuing a verifiable Medical Fitness Certificate
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2">
                    Failed to Issue Medical Fitness Certificate
                  </h3>
                    ...
                    <div className="max-w-2xl mx-auto mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-left">
                      ...
                      <div className="text-sm text-yellow-800 space-y-2">
                        <p className="font-medium">
                          To issue Medical Fitness Certificate credentials, you need to configure the credential definition ID:
                        </p>
                        <ol className="list-decimal ml-5 space-y-2">
                          ...
                          <li>
                            Add or update: <code className="bg-yellow-100 px-2 py-1 rounded">MEDICAL_FITNESS_CRED_DEF_ID=your-credential-definition-id</code>
                          </li>
                          <li>
                            Also configure: <code className="bg-yellow-100 px-2 py-1 rounded">MEDICAL_CENTER_ORG_ID</code>, <code className="bg-yellow-100 px-2 py-1 rounded">MEDICAL_CENTER_CLIENT_ID</code>, and <code className="bg-yellow-100 px-2 py-1 rounded">MEDICAL_CENTER_CLIENT_SECRET</code>
                          </li>
                          <li>
...
export default function IssuingMedicalFitnessPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Loading...</div>}>
      <IssuingMedicalFitnessContent />
    </Suspense>
  );
}
