...
export default function OnboardingEKYCConnectPage() {
  ...
  useEffect(() => {
    ...
    console.log('[OnboardingEKYC] WebSocket status update:', wsStatus, connectionData);
      ...
      if (wsStatus === "active" || wsStatus === "completed") {
        if (connectionStatus === "connecting") {
          ...
          if (connectionData?.connectionId) {
            ...
            console.log('[OnboardingEKYC] Connection established with connectionId:', connectionData.connectionId);
  ...
  const initiateConnection = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/connections/session', {
        ...
        body: JSON.stringify({ requestType: 'onboarding' }),
      });
  ...
  const sendProofRequest = async (sessionId: string, connectionId: string) => {
    console.log('[OnboardingEKYC] Sending proof request', { sessionId, connectionId });
    setConnectionStatus("requesting-proof");
    setConnectionMessage("Requesting your eKYC credentials...");

    try {
      const response = await fetch('/api/finance/onboarding-ekyc/request-proof', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        console.log('[OnboardingEKYC] Proof request sent successfully', data.data);
        ...
        console.error('[OnboardingEKYC] Failed to send proof request:', data.error);
        setConnectionMessage("Failed to request credentials. Please try again.");
        setConnectionStatus("connected");
      }
    } catch (error) {
      console.error('[OnboardingEKYC] Error sending proof request:', error);
  ...
  const verifyProof = async (proofIdToVerify: string) => {
    console.log('[OnboardingEKYC] Verifying proof presentation', { proofId: proofIdToVerify });
    setIsVerifying(true);
    ...
    try {
      ...
      console.log('[OnboardingEKYC] Verify API response:', { status: response.status, data });
      ...
      if (data.success && data.data?.isVerified === true && data.data?.state === 'done') {
        console.log('[OnboardingEKYC] Proof verification successful', data.data);
        setConnectionMessage("Credentials verified! Redirecting...");

        // Navigate immediately after successful verification
        router.push(`/finance/onboarding-ekyc/onboarding?sessionId=${sessionId}`);
      } else {
        ...
        console.error('[OnboardingEKYC] Proof verification failed:', errorMsg, data);
        setConnectionMessage(`Failed to verify credentials: ${errorMsg}`);
        setConnectionStatus("connected");
        setIsVerifying(false);
      }
    } catch (error) {
      console.error('[OnboardingEKYC] Error verifying proof:', error);
  ...
  useEffect(() => {
    if (connectionData?.eventType === "proof") {
      console.log('[OnboardingEKYC] Proof status update:', connectionData);
      ...
      if (connectionData.status === "presentation-received" && !isVerifying) {
        ...
        if (proofIdToVerify) {
          console.log('[OnboardingEKYC] Triggering verification for proofId:', proofIdToVerify);
          verifyProof(proofIdToVerify);
        } else {
          console.error('[OnboardingEKYC] No proofId available for verification');
  ...
  const qrCodeValue = invitationUrl || `confirmd://connect?sessionId=${sessionId}&requestType=onboarding`;
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-6">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">Customer Onboarding (eKYC)</h2>
                <p className="text-blue-100 mt-1">
                  Connect your digital wallet to verify your identity
                </p>
              </div>
              <Link
              ...
              <div className="text-center py-12">
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-600 mb-8">
                    ...
                    wallet app to automatically share your eKYC information.
                  </p>

                  <button
                    onClick={initiateConnection}
                    disabled={isLoadingInvitation}
                    className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                  >
                ...
                <div className="max-w-md mx-auto">
                  ...
                  {sessionId && (
                    <button
                      ...
                      className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-lg font-semibold rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl"
                    >
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-800">
            This demo verifies your eKYC credentials to confirm your identity. Once verified,
            you'll be able to complete your account opening application securely.
          </p>
        </div>
      </div>
    </div>
