...
export default function InvestmentAccountConnectPage() {
  ...
  useEffect(() => {
    ...
    console.log('[InvestmentAccount] WebSocket status update:', wsStatus, connectionData);
      ...
      if (wsStatus === "active" || wsStatus === "completed") {
        if (connectionStatus === "connecting") {
          ...
          if (connectionData?.connectionId) {
            ...
            console.log('[InvestmentAccount] Connection established with connectionId:', connectionData.connectionId);
  ...
  const initiateConnection = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/connections/session', {
        ...
        body: JSON.stringify({ requestType: 'investment' }),
      });
  ...
  const sendProofRequest = async (sessionId: string, connectionId: string) => {
    console.log('[InvestmentAccount] Sending proof request', { sessionId, connectionId });
    setConnectionStatus("requesting-proof");
    setConnectionMessage("Requesting your eKYC credentials...");

    try {
      const response = await fetch('/api/finance/investment-account/request-proof', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        console.log('[InvestmentAccount] Proof request sent successfully', data.data);
        ...
        console.error('[InvestmentAccount] Failed to send proof request:', data.error);
        setConnectionMessage("Failed to request credentials. Please try again.");
        setConnectionStatus("connected");
      }
    } catch (error) {
      console.error('[InvestmentAccount] Error sending proof request:', error);
  ...
  const verifyProof = async (proofIdToVerify: string) => {
    console.log('[InvestmentAccount] Verifying proof presentation', { proofId: proofIdToVerify });
    setIsVerifying(true);
    ...
    try {
      ...
      console.log('[InvestmentAccount] Verify API response:', { status: response.status, data });
      ...
      if (data.success && data.data?.isVerified === true && data.data?.state === 'done') {
        console.log('[InvestmentAccount] Proof verification successful', data.data);
        setConnectionMessage("Credentials verified! Redirecting...");

        // Navigate immediately after successful verification
        router.push(`/finance/investment-account/success?sessionId=${sessionId}`);
      } else {
        ...
        console.error('[InvestmentAccount] Proof verification failed:', errorMsg, data);
        setConnectionMessage(`Failed to verify credentials: ${errorMsg}`);
        setConnectionStatus("connected");
        setIsVerifying(false);
      }
    } catch (error) {
      console.error('[InvestmentAccount] Error verifying proof:', error);
  ...
  useEffect(() => {
    if (connectionData?.eventType === "proof") {
      console.log('[InvestmentAccount] Proof status update:', connectionData);
      ...
      if (connectionData.status === "presentation-received" && !isVerifying) {
        ...
        if (proofIdToVerify) {
          console.log('[InvestmentAccount] Triggering verification for proofId:', proofIdToVerify);
          verifyProof(proofIdToVerify);
        } else {
          console.error('[InvestmentAccount] No proofId available for verification');
  ...
  const qrCodeValue = invitationUrl || `confirmd://connect?sessionId=${sessionId}&requestType=investment`;
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-6">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">Open Investment Account</h2>
                <p className="text-purple-100 mt-1">
                  Reuse your eKYC credential for instant account opening
                </p>
              </div>
              <Link
              ...
              <div className="text-center py-12">
                <div className="max-w-md mx-auto">
                  ...
                  <button
                    ...
                    className="bg-purple-600 hover:bg-purple-700 text-white font-semibold px-8 py-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
                  >
        ...
        <div className="mt-6 bg-purple-50 border border-purple-200 rounded-lg p-4">
          ...
          <p className="text-sm text-purple-800">
            Your eKYC credential was issued once by your primary bank but can be reused here for investment
