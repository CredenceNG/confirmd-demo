...
/**
 * POST /api/finance/investment-account/request-proof
 * Create and send a proof request for investment account opening (eKYC verification)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info("API: Creating proof request for investment account opening", {
      sessionId,
      connectionId,
    });
    ...
    const customComment = "Investment Account Opening - eKYC Verification";
    ...
    logger.error("API: Error creating investment account proof request", {
      error: error.message,
      stack: error.stack,
    });
